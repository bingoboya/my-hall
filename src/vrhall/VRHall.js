...
export class VRHall {
  ...
  _init() {
    ...
    // 相机
    this._camera = new THREE.PerspectiveCamera(50, width / height, 0.1, 10000);
    ...
    // this._controls.target.set(0, 0, 0);   // 初始围绕原点旋转，可按需改
    // this._controls.enableDamping = true;   // 惯性阻尼，手感顺滑
    // this._controls.dampingFactor = 0.05;
    // this._controls.screenSpacePanning = true;  // 右键/双指平移沿屏幕平面移动（不会上下乱跑）
    ...
    this._controls.enableZoom = false;   // 完全接管滚轮
    this._controls.minDistance = 0;
    this._controls.maxDistance = Infinity;
    this._controls.update();

    this._options.container.addEventListener('wheel', this._onWheel.bind(this), { passive: false });
  }
...
_onWheel(event) {
  ...
  this._camera.getWorldDirection(direction); // 直接取相机自身朝向，不依赖 target

  const distance = this._camera.position.distanceTo(this._controls.target);
  const step = Math.max(distance * 0.1, 0.5) * (event.deltaY > 0 ? -1 : 1);

  // 只移动相机
  this._camera.position.addScaledVector(direction, step);

  // target 始终固定在相机前方同等距离，旋转轴永远稳定
  const TARGET_DISTANCE = 5; // 可按场景大小调整
  this._controls.target.copy(this._camera.position).addScaledVector(direction, TARGET_DISTANCE);
  ...
  _getBoxRaycaster({ x, y }, meshes) {
    ...
    if (intersect) {
      ...
      mesh.traverse((child) => {
        if (child.isMesh) {
          console.log('子物体', child.name, ":", child);
          window.currentSelectedMesh = child;
          // child.odata = odataMesh.odata;
        }
      });
  ...
  async _lookat() {
    console.log("初始化视角");
    if (!this._options.cameraOption) return;
    const { position, lookAt } = this._options.cameraOption;
    this._camera.position.set(position.x, position.y, position.z);
    this._controls.target.set(lookAt.x, lookAt.y, lookAt.z);
    this._controls.update();
  ...
  _mouseUp(event) {
    ...
    if (rayRes) {
      ...
      console.log("点击地面移动-rayRes");
      // console.log("点击地面移动-rayRes", mesh, rayRes, position, {
      //   x: event.clientX - left,
      //   y: event.clientY - top,
      // });
